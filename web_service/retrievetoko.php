...
$query1 = "SELECT * FROM loading WHERE user_id = '$uid ' AND tanggal = '$tanggal'";
...
if ($cek > 0) {
	...
	$query3 = "SELECT * FROM toko WHERE id_toko = '$tokoid[$y]' AND (status = 'Check In' OR status = 'Pending')";
	...
	while ($get3 = mysqli_fetch_object($find3)) {
		...
		$field["longitude"] = $get3->longitude;
        $field["latitude"] = $get3->latitude;
