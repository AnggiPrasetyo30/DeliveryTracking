...
import com.example.monitorbudimas.Adapter.RecyclerTrack;
...
public class Track extends Fragment {
    private RecyclerTrack adDatat;
    private final List<mobil> itemt = new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        RecyclerView recyclerView = view.findViewById(R.id.viewt);
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
        adDatat = new RecyclerTrack(getActivity(), itemt);
        recyclerView.setAdapter(adDatat);
    ...
    public void retrievedata() {
        ...
        tampilData.enqueue(new Callback<responseMobil>() {
            ...
            public void onResponse(Call<responseMobil> call, Response<responseMobil> response) {
                ...
                final List<mobil> itemmobillist = response.body().getData();

                if (itemmobillist != null) {
                    itemt.addAll(itemmobillist);
                    adDatat.notifyDataSetChanged();
                }else{
